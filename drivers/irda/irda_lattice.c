...
static struct msm_gpiomux_config msm_irda_configs[] = {
	{
		.gpio      = 0,		/* BLSP1 QUP4 SPI_DATA_MOSI */
		.settings = {
	...
	{
		.gpio      = 1,		/* BLSP1 QUP4 SPI_DATA_MISO */
		.settings = {
			[GPIOMUX_ACTIVE] = &gpio_spi_config,
			[GPIOMUX_SUSPENDED] = &gpio_spi_susp_config,
		},
...
#define GPIO_3V3_EN             56
//#define GPIO_1V2_EN             67
#define GPIO_RESET_IR           55
#define GPIO_CDONE_IR           54
...
/*
  ...
  */
	...
	*/
	...
	if(ir_clk == NULL){
		printk(KERN_INFO "ice40: Lattice BBCLK2 clk get failed \n");
		
		goto initialize_faled;
	}
	else
	{
		printk(KERN_INFO "ice40: Lattice BBCLK2 clk get successfully \n");
