...
/*
  * modfiy code  for ar0542 standby mode
  *
...
#define 	ACTUTOR_MODULE_ID_SIKAO 0X01
#define 	ACTUTOR_MODULE_ID_MITSUMI 0X10
#define 	ACTUTOR_MODULE_ID_JCT 0X11
#endif
...
/*******************************************************************************
* otp_type: otp_type should be 0x30,0x31,0x32,0x34
* return value:
...
void ar0542_read_module_id(struct msm_sensor_ctrl_t *s_ctrl)
{
...
#if 1	
	module_info[0].actuator_id = st_ar0542_otp.vcm_maker_info;
	switch ( module_info[0].actuator_id) {
	case 0x01:
		sprintf(buf, "shicoh");
		break;
	case 0x10:
	...
	default:
        ...
        snprintf(module_info[0].actuator_name, 64, "actuator name: %s_dw9714", buf);
	snprintf(module_info[0].flash_name, 64, "flash name: lm3642");
        snprintf(module_info[0].resolution_raw_format, 64, "resolution: 5M_AF_bayer");
	module_info[0].chip_id = s_ctrl->sensordata->slave_info->sensor_id;
#endif
}
#else
...
void ar0542_read_module_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	case SENSOR_INFO_MODULE_ID_QETCH:
		...
		s_ctrl->sensordata->module_name = "qtech_ar0542_mirror2flip";
		break;
	case SENSOR_INFO_MODULE_ID_SUNNY:
		s_ctrl->module_name = "sunny_ar0542";
		s_ctrl->sensordata->module_name = "sunny_ar0542_mirror2flip";
		break;
	case SENSOR_INFO_MODULE_ID_MCNEX:
		s_ctrl->module_name = "mcnex_ar0542";
		s_ctrl->sensordata->module_name = "mcnex_ar0542_mirror2flip";
		break;
	case SENSOR_INFO_MODULE_ID_TRULY:
		s_ctrl->module_name = "truly_ar0542";
		s_ctrl->sensordata->module_name = "truly_ar0542_mirror2flip";
		break;
	case SENSOR_INFO_MODULE_ID_SAMSUNG:
		s_ctrl->module_name = "samsung_ar0542";
		s_ctrl->sensordata->module_name = "samsung_ar0542_mirror2flip";
		break;
	case SENSOR_INFO_MODULE_ID_KARR:
		s_ctrl->module_name = "karr_ar0542";
		s_ctrl->sensordata->module_name = "karr_ar0542_mirror2flip";
		break;	
	default:
		s_ctrl->module_name = "ar0542";
		s_ctrl->sensordata->module_name = "ar0542_mirror2flip";
		break;
	}
	s_ctrl->sensordata->default_module_name = "ar0542_mirror2flip";	
...
void ar0542_read_module_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	default:
		s_ctrl->module_name = "ar0542";
		s_ctrl->sensordata->module_name = "ar0542";
		break;
	}
	pr_err("%s: module_name(project command)   = %s \n",__func__,s_ctrl->module_name);
	pr_err("%s: module_name(relate lib_name)   = %s \n",__func__,s_ctrl->sensordata->module_name);
...
/*********make sure that the state is default***********/
	{
 ...
 int32_t get_actuator_cam_name( int *cam_name)
{
	...
	if (0x01== vcm_ic_id){//dw9714 ic
		switch(vcm_maker_id){
		case ACTUTOR_MODULE_ID_SIKAO:
			sprintf(buf, "dw9714_sikao");
			*cam_name = 0;
			break;
		case ACTUTOR_MODULE_ID_JCT:
			...
			*cam_name = 2;
			break;
		case ACTUTOR_MODULE_ID_MITSUMI:
			sprintf(buf, "dw9714_mitsumi");
			*cam_name = 0;
			break;
		default:
			sprintf(buf, "dw9714");
		...
		pr_err("wly: %s:  actuator_name=%s, cam_name=%d\n",__func__,buf, *cam_name);
	}
	return 0;
}
